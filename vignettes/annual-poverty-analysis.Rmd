...
1. Build a **crosswalk** that identifies which reference month each observation belongs to (using quarterly data)
2. **Apply** this crosswalk to annual income data using `pnadc_apply_periods()` (which also calibrates weights)
...
- How to apply the crosswalk to annual PNADC income data and calibrate weights
...
The solution is a **crosswalk**: we run mensalization on quarterly data to get reference months, then apply the crosswalk to annual data using `pnadc_apply_periods()`. This function handles the merge internally and also calibrates survey weights.

### Join Keys

The `pnadc_apply_periods()` function uses five key variables to link crosswalk records with your data:

```{r join-keys}
# Join keys used internally by pnadc_apply_periods()
# Note: Column names are case-sensitive
join_keys <- c("Ano", "Trimestre", "UPA", "V1008", "V1014")
```

> **Note:** You don't need to call `merge()` directly. The `pnadc_apply_periods()` function uses these keys internally to match crosswalk records with your data.

### Match Rate and Missing Observations

When applying the crosswalk to annual data, expect approximately **97% of observations** to have a determined reference month. The crosswalk built from 2015-2024 quarterly data (40 quarters) achieves **96.3% month determination** on the quarterly data itself. When applied to annual data with `anchor = "year"`, this translates to a **96.9% match rate** due to the yearly weight calibration structure.

The remaining ~3% occur because:
...
> **Want even higher determination rates?** Experimental probabilistic strategies can boost rates further (e.g., 96.3% → 97-98% for months). See the [experimental strategies section](how-it-works.html#experimental-strategies) in the How It Works vignette. For most analyses, the strict determination rates above are already excellent.
...
| `UPA`, `V1008`, `V1014` | Household-panel join keys |
| `V2008`, `V20081`, `V20082` | Birthday (day, month, year) |
| `V2009` | Age |
| `V1028` | Original survey weight (quarterly) |
...
The function reports the match rate and details about the hierarchical calibration process.

> **Why use `pnadc_apply_periods(..., anchor = "year")`?**
>
> Annual PNADC data contains only one visit per household (e.g., visit 1 or visit 5),
> not all 8 rotation groups like quarterly data. The function properly calibrates the
> annual weight **V1032** to monthly SIDRA population totals using `anchor = "year"`
> to preserve yearly totals while redistributing to months.
>
> For technical details, see [How It Works](how-it-works.html#mensalizing-annual-pnadc-data).
...
# Survey design with monthly weights (March 2020 example)
design <- svydesign(
  ids = ~UPA, strata = ~estrato, weights = ~peso,
  data = d_monthly[ref_month_yyyymm == 202003], nest = TRUE
)

# Poverty rate with confidence interval
svymean(~I(hhinc_pc < poverty_line), design, na.rm = TRUE)
```

For detailed guidance on variance estimation with monthly weights, see the [Complex Survey Design](complex-survey-design.html) vignette.
