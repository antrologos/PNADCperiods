...
When merging the crosswalk with annual data, expect approximately **97% of observations** to have a determined reference month. The crosswalk built from 2015-2024 quarterly data (40 quarters) achieves **96.3% month determination** on the quarterly data itself. When applied to annual data with `anchor = "year"`, this translates to a **96.9% match rate** due to the yearly weight calibration structure.
...
#> [1] 0.963  # 96.3% of observations have determined reference months (40 quarters)
```
...
#> Applying reference period crosswalk...
#>   Matched 3,978,364 of 4,105,414 observations (96.9%)
#>   Calibrating month weights (anchor: year)...
#>     Fetching population targets from SIDRA...
#>     Auto-selected 4 cell level(s) for ref_month_yyyymm calibration
#>     Skipping celula3: min cell size 4 < threshold 10
#>     Applied 2 hierarchical cell level(s)
#>     Calibrated 3,978,364 observations
#> Done.
