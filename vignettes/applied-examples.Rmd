...
| Fortnightly | ~7% | ~3,000 | Transition timing |
...
1. **Probabilistic assignment**: When the interview window spans exactly two consecutive periods and ≥90% of the interval falls in one period, assign to that period.

2. **UPA aggregation**: When ≥50% of observations within a UPA have the same determined period, extend that assignment to undetermined observations.

With both strategies (`strategy = "both"`):

| Granularity | Strict Rate | Experimental Rate | Improvement |
|-------------|-------------|-------------------|-------------|
| Fortnight | ~6% | ~28% | ~5× more data |
| Weekly | ~1.2% | ~2% | ~1.6× more data |
...
# The experimental crosswalk now has ~28% fortnight determination vs ~6% strict
...
ggplot() +
  ...
  labs(
    ...
    caption = "Uses pnadc_experimental_periods() for ~28% fortnight determination vs ~6% strict rate."
  ) +
  theme_minimal(base_size = 11)
```
...
With strict determination, the ~1.2% weekly rate makes this analysis challenging. However, using `pnadc_experimental_periods()` boosts this to ~2%. We **pool multiple years** (2019, 2022-2025, excluding COVID years) and compare Carnival states to other states. We use **average hours worked** (VD4035) as our indicator, which should show a clear drop during Carnival:
...
ggplot(weekly_carnival,
  ...
  labs(
    ...
    fill = "Region",
    caption = "Uses pnadc_experimental_periods() for improved weekly determination. Data pooled across 5 years."
  ) +
  theme_minimal(base_size = 11)
```
...
1. **Monthly analysis handles most use cases** with ~95% strict determination rate
2. **Fortnightly analysis** is more practical with `pnadc_experimental_periods()` (~28% rate vs ~6% strict)
3. **Weekly analysis** benefits from experimental strategies (~2% vs ~1.2% strict); still requires pooling years for subgroup analysis
