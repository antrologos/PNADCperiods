...
The `PNADCperiods` package solves this problem by identifying which specific month each interview refers to, using birthday information and IBGE's interview scheduling rules. With the full PNADC history (2012-2025), the algorithm achieves a **95.4% determination rate** for monthly identification---meaning we can assign specific reference months to nearly all observations.
...
|-------------|-------------------|-------------------|----------|
| Monthly | **95.4%** | ~130,000 | Most analyses |
| Fortnightly | **7.1%** | ~3,000 | Transition timing |
| Weekly | **2.5%** | ~1,000 | Shock detection |
...
|-------------|-------------|-------------------|-------------|
| Fortnight | 7.1% | **~11%** | ~1.5× more data |
| Weekly | 2.5% | **~6%** | ~2.4× more data |
...
# For fortnightly analysis, we need CALIBRATED weights, not V1028.
# The fortnight-determined subset (~12-13%) is NOT a random sample,
# so using uncalibrated V1028 would cause systematic bias.
# We use pnadc_apply_periods() with calibration_unit = "fortnight"
# to generate properly calibrated weight_fortnight values.

# First merge the experimental crosswalk for fortnight columns
...
# Calibrate fortnight weights using experimental crosswalk
...
# Filter to COVID period with calibrated weights
pnadc_fortnight <- pnadc_fortnight[!is.na(ref_fortnight_in_quarter) & !is.na(weight_fortnight) &
                                   Ano == 2020 & Trimestre %in% 1:2]

# Compute participation rate by fortnight using CALIBRATED weight_fortnight
# This ensures proper population representation for the non-random fortnight subset
...
# For weekly analysis, we need CALIBRATED weights, not V1028.
# The weekly-determined subset (~5-6%) is NOT a random sample,
# so using uncalibrated V1028 would cause systematic bias.
# We use pnadc_apply_periods() with calibration_unit = "week"
# to generate properly calibrated weight_weekly values.
...
# Calibrate weekly weights using experimental crosswalk
...
# Filter to employed with valid hours and calibrated weights
pnadc_weekly <- pnadc_weekly[
  !is.na(ref_week_in_quarter) & !is.na(weight_weekly) &
  Trimestre == 1L & Ano %in% carnival_years &
...
# Use CALIBRATED weight_weekly for proper population representation
