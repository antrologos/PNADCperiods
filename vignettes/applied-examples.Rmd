...
When the minimum wage increases, **habitual income captures the change immediately**, while **effective income shows it with a one-month lag**. This timing difference is invisible in quarterly data but becomes clear with monthly mensalization.

### Methodology

We track the proportion of workers earning **exactly** each historical minimum wage value (not a band):

- **Population**: Formal private sector employees (VD4009 == 1), age >= 18
- **All analyses weighted by `weight_monthly`** (adjusted sampling weights)
- Each MW value (R$ 622, R$ 678, ..., R$ 1,518) is tracked as a separate series

```{r mw-analysis-setup, message=FALSE}
# Load exact minimum wage tracking data
mw_exact <- as.data.table(vignette_monthly_mw_exact)
mw_history <- as.data.table(vignette_mw_history)

# Create long format for plotting
mw_long <- melt(mw_exact,
...
# Get adjustment months for reference
adj_months <- vignette_mw_adjustment_months
adj_dates <- as.Date(paste0(adj_months %/% 100, "-", adj_months %% 100, "-15"))
```

### The Big Picture: All MW Values (2012-2025)

Each line shows the proportion of workers earning exactly one specific MW value. When MW changes, we see the old value decline while the new value rises.

```{r mw-big-picture, fig.height=7, fig.width=12}
# Color palette for MW values
mw_values <- sort(unique(mw_exact$mw_value))
n_colors <- length(mw_values)
mw_colors <- setNames(
  colorRampPalette(c("#2166ac", "#67a9cf", "#d1e5f0", "#fddbc7", "#ef8a62", "#b2182b"))(n_colors),
  as.character(mw_values)
)

# Filter to show only MW values with meaningful presence (>0.1% at some point)
mw_long_filtered <- mw_long[, max_pct := max(pct_at_mw), by = mw_value][max_pct > 0.001]

ggplot(mw_long_filtered, aes(x = period, y = pct_at_mw,
                              color = factor(mw_value), group = mw_value)) +
...
### Primary Example: First Transition (October 2012 - April 2013)

The cleanest demonstration of the one-month lag. In January 2013, MW increased from R$ 622 to R$ 678:

```{r mw-2013-transition, fig.height=5, fig.width=10}
# Filter to the transition period
mw_2013 <- mw_long[period >= "2012-10-15" & period <= "2013-04-15" &
                   mw_value %in% c(622, 678)]

# Adjustment date
adj_jan_2013 <- as.Date("2013-01-15")

ggplot(mw_2013, aes(x = period, y = pct_at_mw,
                    color = factor(mw_value), linetype = factor(mw_value))) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2.5) +

  # Mark the adjustment
  geom_vline(xintercept = adj_jan_2013, linetype = "dashed", linewidth = 0.8, color = "gray40") +
...
**Key observation**:
- In **January 2013**: Habitual income shows workers shifting from R$ 622 to R$ 678
- In **February 2013**: Effective income shows the same shift (1-month lag)

### Special Case: Multi-Year Period with Mid-Year Adjustments (Oct 2019 - Apr 2023)

This period includes several interesting events:
- January 2020: R$ 998 → R$ 1,039
- February 2020: R$ 1,039 → R$ 1,045 (correction)
- May 2023: R$ 1,302 → R$ 1,320 (mid-year adjustment)

```{r mw-2019-2023, fig.height=6, fig.width=12}
# Filter to the multi-year period
mw_special <- mw_long[period >= "2019-10-15" & period <= "2023-04-15" &
                      mw_value %in% c(998, 1039, 1045, 1100, 1212, 1302, 1320)]

# Adjustment dates in this period
adj_special <- adj_dates[adj_dates >= "2019-10-15" & adj_dates <= "2023-04-15"]

ggplot(mw_special, aes(x = period, y = pct_at_mw,
                       color = factor(mw_value), group = mw_value)) +
...
```{r mw-summary-table}
# For each adjustment, compute the share at new MW in adjustment month
# Habitual shows it immediately, effective shows it the next month

# Get unique adjustment months from history
adj_info <- mw_history[mw_change == TRUE]

# For each adjustment, get the new MW value and compute shares
summary_list <- lapply(seq_len(nrow(adj_info)), function(i) {
  adj_yyyymm <- adj_info$yyyymm[i]
  new_mw <- adj_info$mw_value[i]

  # Current month data
  curr <- mw_exact[ref_month_yyyymm == adj_yyyymm & mw_value == new_mw]

  # Next month
  next_yyyymm <- if (adj_yyyymm %% 100 == 12) {
    (adj_yyyymm %/% 100 + 1) * 100 + 1
  } else {
    adj_yyyymm + 1
  }
  next_data <- mw_exact[ref_month_yyyymm == next_yyyymm & mw_value == new_mw]

  if (nrow(curr) > 0 && nrow(next_data) > 0) {
    data.table(
      Month = format(as.Date(paste0(adj_yyyymm %/% 100, "-", adj_yyyymm %% 100, "-01")), "%b %Y"),
      `New MW (R$)` = new_mw,
      `Habitual (adj month)` = sprintf("%.1f%%", curr$pct_habitual * 100),
      `Effective (adj month)` = sprintf("%.1f%%", curr$pct_effective * 100),
      `Effective (+1 month)` = sprintf("%.1f%%", next_data$pct_effective * 100)
    )
  } else {
    NULL
  }
...
summary_dt <- rbindlist(summary_list[!sapply(summary_list, is.null)])

knitr::kable(summary_dt,
             caption = "MW Adjustments: Share at New MW Value. Habitual captures change immediately; Effective shows 1-month lag.",
             align = c("l", "r", "r", "r", "r"))
```

### Conclusion

The one-month lag between habitual and effective income at MW adjustments demonstrates:

1. **Habitual income (VD4016)** captures wage changes in the **current reference month**
2. **Effective income (VD4017)** reflects wages from the **previous month**
3. This pattern is **invisible in quarterly data** but clear with monthly mensalization
4. For policy evaluation of minimum wage impacts, the **correct timing matters**
