...
|========================================================| 100%
  Determination rate: 97.0%
  Observations with monthly weights: 10,523,189
```
...
- **Annual PNADC data**: Working with annual income data? See [Monthly Poverty Analysis with Annual PNADC Data](annual-poverty-analysis.html) for the workflow using `mensalize_annual_pnadc()`.
...
```r
mensalizePNADC(
  data,                    # Stacked PNADC data.table/data.frame
  compute_weights = FALSE, # Compute monthly calibrated weights?
  keep_all = TRUE,         # Return all rows (including indeterminate)?
  output = "crosswalk",    # "crosswalk", "microdata", or "aggregates"
...
- `"crosswalk"` (default): Returns join keys + reference month columns + weight_monthly
- `"microdata"`: Returns full input data with all computed columns attached
- `"aggregates"`: Returns monthly aggregated indicators (requires `compute_weights = TRUE`)
...
| `mensalize_annual_pnadc()` | Merge crosswalk with annual data + calibrate V1032 weights |
